...
- [ ] 2.2 Rewrite the README permission-contract paragraph ("declares no client capabilities… never get permission prompts") to state the headless allow-all posture (prefer `AllowAlways`, fall back to any offered allow option), the `AllowAlways` persistence side effect, and that elicitation/fs/terminal remain unsupported. Verify: README grep finds no stale deny-all wording
...
- [ ] 7.1 Add the typed-results surface to types/ponos.d.luau: `result` on `SessionOptions` typed as an optional string-keyed table (the declared JSON Schema) and `result` on `PromptResult` as an optional field holding the converted submission value (`nil` when absent). Verify: the `ponos types` byte-identical sync test stays green
- [ ] 7.2 Extend tests/fixtures/types_probe.luau with a result session (small schema, `MOCK_SUBMIT`) asserting the outcome's `result` field end to end. Verify: `cargo test --test types` passes and the `ponos-analyze` nix check (luau-lsp strict mode) stays green

## 8. Degradation, concurrency, example, docs

- [ ] 8.1 Concurrency coverage: e2e test with two result sessions of different schemas running prompts concurrently. Verify: each `out.result` validates against its own schema only
- [ ] 8.2 Add `examples/typed_results.luau` (schema module + review-style flow incl. a nil-check retry loop) and its explicit test entry in tests/examples.rs. Verify: `cargo test --test examples typed_results` passes offline via mock-agent
- [ ] 8.3 README: document `result` in the `ponos` namespace table, the outcome `result` field, the Luau↔JSON mapping notes (null→nil, `{}`-as-object, integer/number), `mcp__ponos__result_submit` naming, and the degradation behavior. Verify: README examples run as written
- [ ] 8.4 Full suite + spec sync: `cargo test` green offline, `nix flake check` green, `openspec validate add-typed-agent-results --strict` passes
