...
During a prompt turn ponos SHALL receive `session/update` notifications and: accumulate `agent_message_chunk` into the final message text, track tool call and plan updates for display, and fold `usage_update` into the returned `usage` counts. The turn SHALL complete when the `session/prompt` response arrives, returning its `stopReason` as `stop_reason`.
...
ponos SHALL declare no client capabilities during initialization. Any agent-to-client request ponos has declared no support for — `session/request_permission`, `fs/read_text_file`, `fs/write_text_file`, `terminal/*`, `elicitation/create` — SHALL be answered with a JSON-RPC error indicating the method is unsupported, and MUST NOT block the turn indefinitely.

#### Scenario: Permission request denied
- **WHEN** an agent calls `session/request_permission`
- **THEN** ponos responds with an unsupported-method error; the agent (per protocol) proceeds with its own fallback behavior

#### Scenario: No hanging turns
- **WHEN** an agent issues any unsupported agent-to-client request mid-turn
