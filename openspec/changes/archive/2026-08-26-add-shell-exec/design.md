...
- Outer cancellation rides the same path: `RunConfig` carries a shutdown watch channel (control-plane data, deliberately not a sixth port — nothing world-touching, it only asks the run to stop), which the composition root fires on SIGINT/SIGTERM with the exit code to report (128+signal, shell convention). The run loop races the script future against the channel; on fire it abandons the script future (dropping it drops root-coroutine exec port futures — the kill-on-drop guards fire synchronously) and calls the same teardown, returning the carried code (130/143). A second signal before teardown finishes exits the process immediately. Abandoned entries whose owning coroutine no longer exists are recognized by `Rc::strong_count == 1` (registry-only owner) — dead by construction — so the teardown drain never stalls waiting for a coroutine that was already dropped.
