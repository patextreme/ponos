...
- [x] 1.1 Implement MCP config materialization: filter stdio servers from
...
- [x] 1.2 Add the capability probe (cached `pi --help` substring check for
...
- [x] 1.3 Thread the config through spawning: `PiRpcProcess.spawn` accepts the
...
- [x] 1.4 Run the full pi-acp suite (`npm test`) and lint (`npm run lint`);
  all green

## 2. Patch + flake

- [x] 2.1 Export the diff to `patches/pi-acp-mcp-config.patch` (clean against
  rev `d1cffc0`; `git diff` in the clone) and verify it applies: reset the
  clone to the rev, `git apply` the patch, no conflicts
- [x] 2.2 Add `packages.<system>.pi-acp` to `flake.nix`: `buildNpmPackage`,
...
- [x] 2.3 Add `pi-acp` to `devShell.packages`; verify inside `nix develop`
  that `command -v pi-acp` resolves to the flake output and
  `.ptah/config.toml`'s `pi` agent entry needs no edit
- [x] 2.4 Verify `nix flake check` passes including the new package build

## 3. End-to-end + docs

- [x] 3.1 Manual smoke (gitignored `.work/smoke/`): a `.luau` script with
...
- [x] 3.2 Degradation check: with `PI_ACP_PI_COMMAND` pointed at a fake pi
...
- [x] 3.3 Document in `README.md`: the `pi` agent entry, the
...
- [x] 3.4 `openspec validate --change pi-acp-mcp-wiring` passes and the
