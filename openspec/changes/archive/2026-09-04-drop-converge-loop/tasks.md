...
- [x] 1.1 Inline the convergence loop into
...
- [x] 1.3 Delete `factory-components/std/converge.luau`.

## 2. Docs

- [x] 2.1 `factory-components/README.md`: drop the converge bullets; add
  ...
  change.
- [x] 2.2 `factory-components/std/README.md`: drop the converge bullet.
- [x] 2.3 `README.md`: repoint the `docs/adr/0001` link to the archived
  change; delete `docs/adr/0001-source-mounted-factory-components.md`
  (the directory goes with it).

## 3. Tests

- [x] 3.1 In `crates/ptah-cli/tests/factory_components.rs`, delete the
...
- [x] 3.2 Add `openspec_component_escalation_fails_without_a_fix`: judge
...
- [x] 3.3 Add `openspec_component_iteration_cap_fails`: judge rules reject
  everything with fixable findings → exit 1, stderr contains "did not
  converge within".

## 4. Verification

- [x] 4.1 `cargo test --test factory_components` green.
- [x] 4.2 Full suite green (`cargo test`), including the dogfood workflow
  tests (shims unchanged, proving the compat gate holds).
- [x] 4.3 `openspec validate drop-converge-loop` clean.
