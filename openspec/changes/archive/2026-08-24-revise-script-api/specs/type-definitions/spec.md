...
- **THEN** analysis accepts it, while a non-string-or-boolean entry value (e.g. `config = { model = 42 }`) reports a type error naming the config option type
