...
- [ ] 1.1 Add `full-moon` to `Cargo.toml` (workspace pin consistent with mlua's Luau
  version); verify `cargo build` succeeds offline inside the dev shell.
- [ ] 1.2 Extract pure path-resolution helpers in `src/script/require.rs`
...
- [ ] 2.1 Implement the full-moon walk: parse a file, collect literal
...
- [ ] 2.2 Implement require-graph traversal: BFS/DFS from the entry over literal require
...
- [ ] 2.3 Implement the agent-name lint against a discovered `Registry`; verify unit
  tests: unknown literal name flagged, known name clean, computed argument not flagged.

## 3. Check pipeline (`src/check.rs`)

- [ ] 3.1 Implement the findings type (`path:line:col: message`, severity, summary
...
- [ ] 3.2 Implement luau-lsp invocation: PATH scan for the binary (exit 2 with
...
- [ ] 3.3 Wire the pass pipeline into `check(cfg) -> CheckOutcome` (exit 0/1/2 per the
...
- [ ] 4.1 Add the `check` subcommand to `src/cli.rs` (one positional script, `--no-color`),
...
- [ ] 4.2 Add the `run` pre-flight in `src/cli.rs`: compile + require + agent lints only
...
- [ ] 4.3 Confirm no behavior regressions for valid scripts: `cargo test` (full suite)
  passes, including `tests/examples.rs`.

## 5. Integration tests (`tests/check.rs`)

- [ ] 5.1 Clean-path test: fixture script (strict, known agent, good requires) + stub
  `luau-lsp` (exit 0) on the child PATH; verify exit 0 and no stdout findings.
- [ ] 5.2 Findings tests: syntax-error fixture; unknown-agent fixture; escaping/missing
...
- [ ] 5.3 luau-lsp stub findings test: stub printing a canned `file(1,1): TypeError:`
  line to stderr and exiting 1; verify the line passes through raw and the check exits 1.
- [ ] 5.4 Missing-binary test: empty PATH temp dir; verify exit 2 and the "luau-lsp not
  found" error message.
- [ ] 5.5 Pre-flight integration: `run` with unknown literal agent name exits 1 before
...
- [ ] 6.1 README: add the check section (passes, luau-lsp PATH dependency,
...
- [ ] 6.2 AGENTS.md: extend the exit-code contract note (`2` for `check` also means
...
- [ ] 6.3 Run `openspec validate add-check-subcommand`, `cargo test`, and
