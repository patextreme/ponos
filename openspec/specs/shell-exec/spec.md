...
- **WHEN** a script calls `ptah.exec("sh -c 'echo boom >&2; exit 3'")`
...
A command run via `ptah.exec` SHALL inherit ptah's environment variables and working directory; v1 provides no `cwd` or `env` override options. The child's stdin SHALL be closed (reads return EOF): exec is non-interactive, and a child that prompts on stdin fails fast instead of hanging or touching the user's terminal.

#### Scenario: Environment inheritance
- **WHEN** ptah is running with `EXAMPLE_TOKEN=x` in its environment and a script calls `ptah.exec("printf $EXAMPLE_TOKEN")`
- **THEN** the result's stdout is `x`

#### Scenario: Working directory inheritance
- **WHEN** ptah is invoked from a directory containing `marker.txt` and a script calls `ptah.exec("cat marker.txt")`
- **THEN** the file is read from the invocation directory's cwd

#### Scenario: Interactive child gets EOF, not the terminal
- **WHEN** a script calls `ptah.exec("cat")` (a child that reads stdin until EOF)
- **THEN** the child reads EOF and exits immediately; no input is consumed from ptah's own stdin

### Requirement: Exec is an injected capability, not a sandbox global
Process execution SHALL reach the scripting environment only through a process-runner capability injected by the composition root; the ambient sandbox globals SHALL remain free of subprocess execution (no `os.execute`, no `io`). `ptah.exec` raises a runtime error when no runner was injected. The capability is always injected by the `ptah` CLI — there is no gating flag or config switch; running a ptah script already implies arbitrary shell through the headless allow-all agent posture.
...
- **WHEN** any script run via the `ptah` CLI calls `ptah.exec`
