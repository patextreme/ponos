...
//! the newline-JSON submit/verdict protocol shared by ptah-main and the
//! hidden `ptah __bridge` subcommand.
//!
//! ptah-main binds a per-session socket and offers the agent an MCP
//! server (the bridge) whose single `result_submit` tool relays
//! submissions over that socket. ptah-main validates each submission
//! against the compiled contract ([`ptah_core::contract`]) and answers
...
//!
//! ```text
//! bridge  ->  ptah : {"op":"submit","value":<any JSON>}
//! ptah   ->  bridge: {"ok":true}
//!                   | {"ok":false,"errors":["...","..."]}
//! ```
//!
...
use ptah_core::contract::{ResultContract, SubmissionSink};
use ptah_core::events::SessionEvent;
use ptah_core::ports::EventSink;

/// Filename prefix for per-session result sockets (`ptah-r-<32hex>.sock`).
const SOCKET_PREFIX: &str = "ptah-r-";
...
mod tests {
    use super::*;
    use ptah_core::contract::ResultContract;
    use ptah_core::events::SessionEvent;
    ...
    async fn stale_socket_is_unlinked_and_rebound() {
        let dir = std::env::temp_dir().join(format!("ptah-rc-stale-{}", std::process::id()));
