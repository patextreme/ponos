//! `ponos.exec` / `ponos.json` binding tests against a stub
...
use ponos_core::ports::{EventSink, ExecError, ExecOutcome, ProcessRunner};
...
fn tmpdir(name: &str) -> std::path::PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!("ponos-exec-{}-{name}-{n}", std::process::id()));
...
// ---------------------------------------------------------------------------
// ponos.exec through the stub runner
// ---------------------------------------------------------------------------
...
fn exec_timeout_raises_naming_command_and_budget() {
    ...
    let err = eval(
        &lua,
        r#"
local ok, err = pcall(ponos.exec, "hang", { timeoutMs = 100 })
...
fn exec_opts_must_be_a_table() {
    ...
    let err = eval(
        &lua,
        r#"
local ok, err = pcall(ponos.exec, "ok", 100)
...
fn exec_without_runner_raises_clearly() {
    ...
    let err = eval(
        &lua,
        r#"
local ok, err = pcall(ponos.exec, "ok")
...
fn exec_spawn_failure_raises_and_ends_event() {
    ...
    let err = eval(
        &lua,
        r#"
local ok, err = pcall(ponos.exec, "spawn-fail")
...
fn exec_emits_labeled_lifecycle_events_around_the_call() {
    let sink = Arc::new(RecordingSink::default());
    let lua = exec_lua(Some(Arc::new(StubRunner::default())), sink.clone());
    eval(&lua, r#"local r = ponos.exec("ok")"#).unwrap();
    let events = sink.events.lock().unwrap();
...
fn exec_threads_timeout_ms_to_the_runner() {
    ...
    let runner = Arc::new(StubRunner::default());
    let lua = exec_lua(Some(runner.clone()), sink);
    eval(&lua, r#"ponos.exec("ok", { timeoutMs = 250 })"#).unwrap();
    assert_eq!(
        runner.calls.lock().unwrap().as_slice(),
        &[("ok".into(), Some(250))]
    );
    // nil timeoutMs is absent, not zero.
    eval(&lua, r#"ponos.exec("ok", { timeoutMs = nil })"#).unwrap();
    assert_eq!(
...
fn json_round_trip_with_indent() {
    ...
    eval(
        &lua,
        r#"
local v = ponos.json.parse('{"a":[1,2]}')
assert(v.a[1] == 1 and v.a[2] == 2, "parse shape")
local s = ponos.json.stringify(v, { indent = 2 })
assert(s == "{\n  \"a\": [\n    1,\n    2\n  ]\n}", "stringify: " .. s)
local compact = ponos.json.stringify(v)
assert(compact == '{"a":[1,2]}', "compact: " .. compact)
-- null decodes to nil
local n = ponos.json.parse('{"x":null}')
assert(n.x == nil, "null must be nil")
-- scalars and nested shapes round trip
assert(ponos.json.parse("true") == true)
assert(ponos.json.parse("7") == 7)
assert(ponos.json.parse('"t"') == "t")
"#,
    )
    .unwrap();
}
...
fn json_malformed_input_raises_catchably() {
    ...
    let err = eval(
        &lua,
        r#"
local ok, err = pcall(ponos.json.parse, "{oops")
assert(not ok, "malformed must raise")
error(tostring(err), 0)
"#,
    )
    .unwrap_err();
    assert!(err.to_string().contains("ponos.json.parse"), "{err}");
}
...
fn json_stringify_rejects_non_string_keys() {
    ...
    let err = eval(
        &lua,
        r#"
local ok, err = pcall(ponos.json.stringify, { [0] = "zero" })
...
fn exec_stdout_flows_into_json_parse() {
    ...
    eval(
        &lua,
        r#"
local r = ponos.exec("json")
local items = ponos.json.parse(r.stdout)
