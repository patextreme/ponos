//! Typed-results integration suite: the `ptah __bridge` subcommand, the
...
//! the `ptah` binary, not a test harness); the bridge is exercised
...
/// A hand-rolled stand-in for ptah-main's side of the result channel:
...
async fn bridge_subcommand_round_trips_submits_and_violations() {
    ...
    let mut command = tokio::process::Command::new(ptah_bin());
    command
        .arg("__bridge")
        .env("PTAH_BRIDGE_ADDR", &path)
        .env("PTAH_RESULT_SCHEMA", schema)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
...
local work = ptah.spawn(function() return s:prompt("slow") end)
ptah.sleep(300)
s:cancel()
local r = work:await()
...
fn prompt_on_result_session_passes_text_verbatim() {
    ...
    // passed: no ptah-appended submit instruction (the mock echoes the
...
#[test]
fn result_session_injects_ptah_server_alongside_user_servers() {
    // Task 5.2: `session/new` receives the user's servers plus the
    // injected `ptah` bridge entry.
...
"#,
    ...
    let json_line = stdout
        ...
        .find(|l| l.starts_with("[ptah] ["))
        .expect("mcpServers JSON in output");
    let servers: serde_json::Value =
        serde_json::from_str(json_line.trim_start_matches("[ptah] ")).expect("valid JSON");
    ...
    let injected = &arr[1];
    assert_eq!(injected["name"], "ptah");
    assert!(
        injected["command"]
            .as_str()
            .is_some_and(|c| c.contains("ptah")),
        "injected command is the ptah binary: {injected}"
    );
    let env_names: Vec<&str> = injected["env"]
        ...
        .collect();
    assert!(env_names.contains(&"PTAH_BRIDGE_ADDR"), "{env_names:?}");
    assert!(env_names.contains(&"PTAH_RESULT_SCHEMA"), "{env_names:?}");
...
"#,
    ...
    let listing: serde_json::Value =
        serde_json::from_str(json_line.trim_start_matches("[ptah] ")).unwrap();
...
fn concurrent_result_sessions_stay_independent() {
    ...
    assert_ok(
        ...
        &format!(
            r#"
local a = ptah.agent({mock_a})
local b = ptah.agent({mock_b})
...
local ta = ptah.spawn(function() return sa:prompt("review a") end)
local tb = ptah.spawn(function() return sb:prompt("review b") end)
local ra = ta:await()
local rb = tb:await()
...
ptah.log("schema error observed: " .. tostring(err))
"#,
        mock = mock_agent(&[])
    );
...
fn legacy_result_option_name_declares_no_contract() {
    ...
    // on a contract session (the mock submits only when a `ptah` server
