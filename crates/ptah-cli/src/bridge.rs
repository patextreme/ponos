//! The hidden `ptah __bridge` subcommand: an MCP server over stdio that
//! relays `result_submit` calls to ptah-main over the session's result
//! socket.
//!
//! The agent (MCP client) spawns one bridge per result session, as
//! suggested in `session/new { mcpServers }`. The bridge is stateless
//! beyond its environment: `PTAH_BRIDGE_ADDR` is the session's
//! Unix-domain result socket and `PTAH_RESULT_SCHEMA` the declared JSON
...
//! ptah-main computes by validating the value against the schema. A
...
/// Name of the injected server (agents derive `mcp__ptah__result_submit`).
pub const SERVER_NAME: &str = "ptah";
/// Env var carrying the session's result socket path.
pub const BRIDGE_ADDR_ENV: &str = "PTAH_BRIDGE_ADDR";
/// Env var carrying the declared JSON schema.
pub const RESULT_SCHEMA_ENV: &str = "PTAH_RESULT_SCHEMA";
...
struct BridgeServer {
    ...
    /// Lazily established persistent connection to ptah-main.
...
/// Entry point for `ptah __bridge`. Runs until stdin closes. Returns the
/// process exit code.
...
async fn run_async() -> Result<(), String> {
    let socket = std::env::var_os(BRIDGE_ADDR_ENV)
        ...
        .ok_or("PTAH_BRIDGE_ADDR is not set (this subcommand is spawned by agents)")?;
    let schema_raw = std::env::var(RESULT_SCHEMA_ENV)
        .map_err(|_| "PTAH_RESULT_SCHEMA is not set".to_string())?;
    let schema: serde_json::Value = serde_json::from_str(&schema_raw)
        .map_err(|e| format!("invalid PTAH_RESULT_SCHEMA: {e}"))?;
...
mod tests {
    ...
    fn bridge_wiring_matches_core_defaults() {
        ...
        let cfg = ptah_core::ports::BridgeConfig::ptah_bridge();
