...
use crate::contract::SubmissionSink;
use crate::events::ToolLine;
use crate::text::{LINE_BUDGET, truncate_visible};
...
impl TurnFold {
    ...
    pub fn push_text(&mut self, delta: &str) -> bool {
    ...
    /// submission.
    pub fn settle_turn(&mut self, discard: bool) -> (String, Option<serde_json::Value>) {
...
pub fn submission_sink(fold: Arc<Mutex<TurnFold>>) -> SubmissionSink {
    Arc::new(move |value| {
...
#[derive(Default)]
pub struct PeekInputs<'a> {
    pub kind: Option<&'a ToolKind>,
