...
/// The ponos version string, surfaced by the CLI (`--version`, the
/// `ponos types` header) and the `ponos.version` script binding. Lives
/// here — not in `ponos-cli` — because both the CLI crate and the
/// script crate read it; member manifests share the literal version.
