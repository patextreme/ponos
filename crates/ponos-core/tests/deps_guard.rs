//! Dependency-direction guard for `ponos-core`.
//!
...
//! or adapter coupling to core fails `cargo test -p ponos-core`
...
//! - the adapter crates (`ponos_acp`, `ponos_render`, `ponos_check`,
//!   `ponos_luau`, `ponos_config`, `ponos_result`, `ponos_cli`) in any
...
const RULE_PROBES: &[(&str, &str)] = &[
    ...
    ("src/text.rs", "use ponos_acp::Transport;\n"),
...
#[test]
fn ponos_core_is_io_free_and_adapter_free() {
    ...
    assert!(
        violations.is_empty(),
        "dependency-direction violations in ponos-core (see the allowlist in \
         crates/ponos-core/tests/deps_guard.rs before widening anything):\n{}",
        violations
            .iter()
            .map(Violation::render)
            .collect::<Vec<_>>()
