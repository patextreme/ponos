...
//!
//! Each ptah session owns one agent subprocess. A driver task runs the
...
//! the sink. ptah declares exactly one client capability — the
...
//! the agent the `ptah __bridge` MCP server in `session/new`; accepted
